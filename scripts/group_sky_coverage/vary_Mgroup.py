...
LogMass_list = np.logspace(12.0, 14.0, 6)
n_samples=1000000

coverage_list = []
for LOG_MASS_THRESHOLD in LogMass_list:
    print("LOG_MASS_THRESHOLD: ", LOG_MASS_THRESHOLD)
    
    # 生成星系团目录
    print("\n1. select galaxy cluster...")
    ...
    # 计算默认参数下的覆盖比例
    print("\n2. 计算覆盖比例...")
    coverage = compute_coverage_fraction_monte_carlo(catalog, R_AP, n_samples, ra_range = (RA_MIN, RA_MAX), dec_range = (DEC_MIN, DEC_MAX))
    print(f"   默认参数 (r_ap={R_AP} Mpc/h): "
          f"覆盖比例 = {coverage:.4f} ({coverage*100:.2f}%)")

    coverage_list.append(coverage)
    
ofile = "./output/group_skycoverage_difflogMgroup.txt"
output = np.array([LogMass_list, coverage_list]).T
header = " logMgroup      sky_coverage"
